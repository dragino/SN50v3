...
size_t macCmdsSize_temp = 0;
bool jr_flag=0;
uint8_t jr_nb=0;
bool DR_small=0;
bool adr_flags=0;
bool address_flags=0;
bool ser_ack=0,ser_ack_record=0,ser_ack_record2=0;
uint8_t printf_channel_mask_onece=1;

extern bool UTC_Request;
 ...
 * measurements
 */
uint16_t LoRaMacDevNonce=1;

static uint32_t joinnonce=0;

static uint16_t downlinkfcnt=0;
/*!
 * Network ID ( 3 bytes )
 */
...
static void OnRadioTxDone( void )
{
    ...
    {
			  void *my_context;
        OnRxWindow2TimerEvent(&my_context);
    }
...
static void PrepareRxDoneAbort( void )
{
    ...
    {
			  void *my_context;
        OnAckTimeoutTimerEvent(&my_context );
    }
    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            LoRaMacRxPayload[0] = macHdr.Value;
						...
						joinoce_temp=(uint32_t)LoRaMacRxPayload[1];
						joinoce_temp|=((uint32_t)LoRaMacRxPayload[2]<<8);
						joinoce_temp|=((uint32_t)LoRaMacRxPayload[3]<<16);
            ...
            if(( micRx == mic )&&(joinnonce!=joinoce_temp))
            {
                LoRaMacDevNonce=LoRaMacDevNonce-1;
							
                LoRaMacJoinComputeSKeys( LoRaMacAppKey, LoRaMacRxPayload + 1, LoRaMacDevNonce, LoRaMacNwkSKey, LoRaMacAppSKey );
							
								joinnonce=joinoce_temp;
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                if( isMicOk == true )
                {
                    ...
                    else
                    {
                        ...
                        {
 													  if((downlinkfcnt!=sequenceCounter)||(sequenceCounter==0))
														{
															SrvAckRequested = true;
															McpsIndication.McpsIndication = MCPS_CONFIRMED;
														  downlinkfcnt=sequenceCounter;
														}
...
void printf_joinmessage(void)
{
	if(debug_flags==1)
	{
		...
		MW_LOG(TS_OFF, VLEVEL_M,"Join Accept:\r\n");		
		MW_LOG(TS_OFF, VLEVEL_M,"Rx1DrOffset:%d\r\n",( LoRaMacRxPayload[11] >> 4 ) & 0x07);
		MW_LOG(TS_OFF, VLEVEL_M,"Rx2Datarate:%d\r\n",LoRaMacRxPayload[11] & 0x0F);
		MW_LOG(TS_OFF, VLEVEL_M,"ReceiveDelay1:%lu ms\r\n",LoRaMacParams.ReceiveDelay1);
		MW_LOG(TS_OFF, VLEVEL_M,"ReceiveDelay2:%lu ms\r\n",LoRaMacParams.ReceiveDelay2);					
	}
}
...
//                txTimeout = true;
        ...
        else  if( ( LoRaMacFlags.Bits.MlmeReq == 1 ) && ( MlmeConfirm.MlmeRequest == MLME_JOIN ) )
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // Sends the same frame again
													  void *my_context;
                            OnTxDelayedTimerEvent(&my_context );
                        }
                    }
                }
                else if( McpsConfirm.McpsRequest == MCPS_CONFIRMED )
                {// Procedure for all other frames
                    if( ( ChannelsNbRepCounter >= LoRaMacParams.ChannelsNbRep ) || (( McpsConfirm.AckReceived == true ) && ( LoRaMacFlags.Bits.McpsInd == 1 )))
                    {
                        ...
                        {
                            UpLinkCounter++;
														if((join_mothod==0)&&(UpLinkCounter!=0))
														{
															*((uint8_t *)(0x2000FE06)) =UpLinkCounter>>24;
															*((uint8_t *)(0x2000FE07)) =UpLinkCounter>>16;
															*((uint8_t *)(0x2000FE08)) =UpLinkCounter>>8;
															*((uint8_t *)(0x2000FE09)) =UpLinkCounter;
															HAL_Delay(50);
														}		
                        }
           ...
           else if(McpsConfirm.McpsRequest == MCPS_UNCONFIRMED)
           {// Procedure for all other frames
        ...
        switch( payload[macIndex++] )
        {
                ...
                {						
                    ...
                    if( ( status & 0x07 ) == 0x07 )
                    {
                        ...
                        LoRaMacParams.ChannelsNbRep = linkAdrNbRep;
												...
												if(debug_flags==1)
												{												
												MW_LOG(TS_OFF, VLEVEL_M,"ADR Message:\r\n");	
											  if(printf_channel_mask_onece==1)
												{
													printf_channel_mask_onece=0;
													#if defined( REGION_US915 )  || defined ( REGION_AU915 ) || defined ( REGION_CN470 )											
													MW_LOG(TS_OFF, VLEVEL_M,"Channel is ");	
													...
													for(int i=0;i<6;i++)
													{
														...
														{
															MW_LOG(TS_OFF, VLEVEL_M,"CH%d_%d ",i*16,i*16+7);
														}
														if((mib.Param.ChannelsMask[i]&0xff00)==0xff00)
														{
															MW_LOG(TS_OFF, VLEVEL_M,"CH%d_%d ",i*16+8,i*16+15);
														}
													}			
													MW_LOG(TS_OFF, VLEVEL_M,"\r\n");
													#endif
													...
													if(DR_small==0)
													{												
														MW_LOG(TS_OFF, VLEVEL_M,"TX Datarate change to %d\r\n",linkAdrDatarate);
														MW_LOG(TS_OFF, VLEVEL_M,"TxPower change to %d\r\n",linkAdrTxPower);
														MW_LOG(TS_OFF, VLEVEL_M,"NbRep change to %d\r\n",linkAdrNbRep);	
														MW_LOG(TS_OFF, VLEVEL_M,"\r\n"); 														
													}
													else
													{
														MW_LOG(TS_OFF, VLEVEL_M,"TX Datarate too small for this payload size, change to %d\r\n",linkAdrDatarate);
														MW_LOG(TS_OFF, VLEVEL_M,"\r\n"); 
														DR_small=0;
													}
											  }
												...
												{
													MW_LOG(TS_OFF, VLEVEL_M,"Received: ADR Message\r\n");		
													MW_LOG(TS_OFF, VLEVEL_M,"\r\n");													
												}												
                    }
            ...
            case SRV_MAC_TX_PARAM_SETUP_REQ:
                {
                    ...
                    if( RegionTxParamSetupReq( LoRaMacRegion, &txParamSetupReq ) != -1 )
                    {
                        ...
                        LoRaMacParams.MaxEirp = LoRaMacMaxEirpTable[txParamSetupReq.MaxEirp];
												...
												getPhy.UplinkDwellTime = LoRaMacParams.UplinkDwellTime;
												phyParam = RegionGetPhyParam( LoRaMacRegion, &getPhy );
												LoRaMacParams.ChannelsDatarate = MAX( LoRaMacParams.ChannelsDatarate, ( int8_t )phyParam.Value );
            ...
            case SRV_MAC_DL_CHANNEL_REQ:
                ...
                {									
										...
										{
											MW_LOG(TS_OFF, VLEVEL_M,"Sync time ok\r");
...
static LoRaMacStatus_t ScheduleTx( void )
{
    ...
    // Select channel
    if( RegionNextChannel( LoRaMacRegion, &nextChan, &Channel, &dutyCycleTimeOff, &AggregatedTimeOff ) == false ) 
    {
        return status;
    }
    ...
    {
        if( ValidatePayloadLength( LoRaMacTxPayloadLen, LoRaMacParams.ChannelsDatarate, macCmdsSize_temp ) == false )
        {
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            adrNext.UplinkDwellTime = LoRaMacParams.UplinkDwellTime;
						adrNext.NbTrans = LoRaMacParams.ChannelsNbRep;
						
            fCtrl->Bits.AdrAckReq = RegionAdrNext( LoRaMacRegion, &adrNext,
                                                   &LoRaMacParams.ChannelsDatarate, &LoRaMacParams.ChannelsTxPower, &LoRaMacParams.ChannelsNbRep, &AdrAckCounter );
            ...
            if( macCmdsSize > 0 ) 
            {
                availableSize = GetMaxAppPayloadWithoutFOptsLength( LoRaMacParams.ChannelsDatarate );
							
                if( macCmdsSize <= LORA_MAC_COMMAND_MAX_FOPTS_LENGTH )
                {
                    LoRaMacCommandsSerializeCmds( LORA_MAC_COMMAND_MAX_FOPTS_LENGTH, &macCmdsSize, MacCommandsBuffer );
                    fCtrl->Bits.FOptsLen = macCmdsSize;
                    // Update FCtrl field with new value of OptionsLength
                    LoRaMacBuffer[0x05] = fCtrl->Value;
                    for( uint8_t i = 0; i <macCmdsSize; i++ )
                    {
                      LoRaMacBuffer[pktHeaderLen++] = MacCommandsBuffer[i];
								...
								{
									LoRaMacCommandsSerializeCmds( availableSize, &macCmdsSize, MacCommandsBuffer );
									framePort = 0;
									payload = MacCommandsBuffer;
									LoRaMacTxPayloadLen = macCmdsSize;
									txpaload_flag=1;
								}
								macCmdsSize_temp = macCmdsSize;
						}
		        else
						{
							macCmdsSize_temp = 0;
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    iwdg_reload();	
		...
		if(jr_flag==1)
		{
		MW_LOG(TS_OFF, VLEVEL_M,"JoinRequest NbTrials= %d\r\n",jr_nb);
		MW_LOG(TS_OFF, VLEVEL_M,"\r\n");			
	  jr_flag=0;
		}
		#endif
    ...
    MW_LOG(TS_OFF, VLEVEL_M,"***** UpLinkCounter= %u *****\n\r", (unsigned int)UpLinkCounter );
    ...
    LoRaMacState |= LORAMAC_TX_RUNNING;
		...
		if((lora_config_otaa_get() == LORA_ENABLE ? 1 : 0)&&(LORA_JoinStatus () == LORA_RESET))
		{						            
		   LoRaMacDevNonce++;
			 save_devNonce_buff();	
		   HAL_Delay(50);
			 MW_LOG(TS_OFF, VLEVEL_M,"DN %d\r\n",LoRaMacDevNonce-1);	
		}
...
LoRaMacStatus_t LoRaMacQueryTxPossible( uint8_t size, LoRaMacTxInfo_t* txInfo )
{
    ...
    uint8_t nbTrans = LoRaMacParams.ChannelsNbRep;
    size_t macCmdsSize = 0;
    ...
    RegionAdrNext( LoRaMacRegion, &adrNext, &datarate, &txPower, &nbTrans, &AdrAckCounter );

    txInfo->CurrentPayloadSize = GetMaxAppPayloadWithoutFOptsLength( datarate );

    LoRaMacCommandsGetSizeSerializedCmds( &macCmdsSize );
		
    // Verify if the MAC commands fit into the FOpts and into the maximum payload.
    if( ( LORA_MAC_COMMAND_MAX_FOPTS_LENGTH >= macCmdsSize ) && ( txInfo->CurrentPayloadSize >= macCmdsSize ) )
    {
        txInfo->MaxPossiblePayload = txInfo->CurrentPayloadSize - macCmdsSize;

        // Verify if the application data together with MAC command fit into the maximum payload.
        if( txInfo->CurrentPayloadSize >= ( macCmdsSize + size ) )
        {
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    switch( mlmeRequest->Type )
    {
        ...
        case MLME_LINK_CHECK:
        {
            ...
            LoRaMacCommandsAddCmd( MOTE_MAC_LINK_CHECK_REQ, macCmdPayload, 0 );
            break;
        }
        case MLME_TXCW:
        {
				...
				case MLME_DEVICE_TIME:
        {
            ...
            // LoRaMac will send this command piggy-pack
            status = LORAMAC_STATUS_OK;
            LoRaMacCommandsAddCmd( MOTE_MAC_DEVICE_TIME_REQ, macCmdPayload, 0 );
            break;
        }
        default:
            break;

...
bool RegionCN779AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint8_t* nbTransOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    if( adrNext->AdrEnabled == true )
    {
      ...
      if( adrNext->AdrAckCounter >= ( CN779_ADR_ACK_LIMIT + CN779_ADR_ACK_DELAY ) )
      {
       ...
       if( adrNext->AdrAckCounter >= ( uint32_t )( CN779_ADR_ACK_LIMIT + ( CN779_ADR_ACK_DELAY << 1 ) ) )
       {
        if( ( ( adrNext->AdrAckCounter - CN779_ADR_ACK_LIMIT ) % CN779_ADR_ACK_DELAY ) == 0 )
        {

...
bool RegionAS923AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint8_t* nbTransOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    if( adrNext->AdrEnabled == true )
    {
      ...
      if( adrNext->AdrAckCounter >= ( AS923_ADR_ACK_LIMIT + AS923_ADR_ACK_DELAY ) )
      {
       ...
       if( adrNext->AdrAckCounter >= ( uint32_t )( AS923_ADR_ACK_LIMIT + ( AS923_ADR_ACK_DELAY << 1 ) ) )
       {
        if( ( ( adrNext->AdrAckCounter - AS923_ADR_ACK_LIMIT ) % AS923_ADR_ACK_DELAY ) == 0 )
        {

...
static uint8_t CountNbOfEnabledChannels( RegionCommonCountNbOfEnabledChannelsParams_t* countNbOfEnabledChannelsParams, uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
...
bool RegionEU868AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint8_t* nbTransOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    if( adrNext->AdrEnabled == true )
    {
      ...
      if( adrNext->AdrAckCounter >= ( EU868_ADR_ACK_LIMIT + EU868_ADR_ACK_DELAY ) )
      {
       ...
       if( adrNext->AdrAckCounter >= ( uint32_t )( EU868_ADR_ACK_LIMIT + ( EU868_ADR_ACK_DELAY << 1 ) ) )
       {
        if( ( ( adrNext->AdrAckCounter - EU868_ADR_ACK_LIMIT ) % EU868_ADR_ACK_DELAY ) == 0 )
        {
...
bool RegionEU868RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    Radio.SetMaxPayloadLength( modem, maxPayload + LORA_MAC_FRMPAYLOAD_OVERHEAD );
		...
		{	
			...
			MW_LOG(TS_OFF, VLEVEL_M, "[%lu]", (long unsigned int)ts); 
			MW_LOG(TS_OFF, VLEVEL_M, "RX on freq %u Hz at DR %d\r\n", (unsigned int)frequency, dr );						
		}
		else
		{
			rxfreq_a=frequency/1000000;
			rxfreq_b=(frequency%1000000)/1000;			
			MW_LOG(TS_OFF, VLEVEL_M, "RX on freq %d.%d MHz at DR %d\r\n",rxfreq_a,rxfreq_b,dr );				
		}
...
//    *txTimeOnAir = Radio.TimeOnAir( modem, txConfig->PktLen );
    if( txConfig->Datarate == DR_7 )
    { // High Speed FSK channel
        *txTimeOnAir = Radio.TimeOnAir( MODEM_FSK, bandwidth, phyDr * 1000, 0, 5, false, txConfig->PktLen, true );
    }
    else
    {
...
bool RegionEU868NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx ) )
    {
        ...
        *aggregatedTimeOff = 0;
        ...
        nbEnabledChannels = CountNbOfEnabledChannels( &countChannelsParams, nextChanParams->Datarate,

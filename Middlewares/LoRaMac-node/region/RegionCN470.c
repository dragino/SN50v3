...
bool RegionCN470AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint8_t* nbTransOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    if( adrNext->AdrEnabled == true )
    {
      ...
      if( adrNext->AdrAckCounter >= ( CN470_ADR_ACK_LIMIT + CN470_ADR_ACK_DELAY ) )
      {
       ...
       if( adrNext->AdrAckCounter >= ( uint32_t )( CN470_ADR_ACK_LIMIT + ( CN470_ADR_ACK_DELAY << 1 ) ) )
       {
        if( ( ( adrNext->AdrAckCounter - CN470_ADR_ACK_LIMIT ) % CN470_ADR_ACK_DELAY ) == 0 )
        {
...
//    *txTimeOnAir = Radio.TimeOnAir( MODEM_LORA, txConfig->PktLen );
    ...
    { // High Speed FSK channel
        *txTimeOnAir = Radio.TimeOnAir( MODEM_FSK, 0, phyDr * 1000, 0, 5, false, txConfig->PktLen, true );
    }
    else
    {

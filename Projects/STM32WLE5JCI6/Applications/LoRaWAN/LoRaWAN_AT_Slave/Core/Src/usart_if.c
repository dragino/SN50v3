...
volatile uint8_t IsUsart1RxPending;
uint8_t aRxBuffer[1];
#endif
/**
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (huart->Instance == LPUART1)
  {
	...
	#if defined (USE_RS485_BL_BASE_BOARD)
	else if (huart->Instance == USART1)
	{
		IsUsart1RxPending=SET;
	}
	#elif defined (USE_SIB_BASE_BOARD)
	else if (huart->Instance == USART1)
	{
		IsUsart1RxPending=SET;
	}	
...
#if defined (USE_RS485_BL_BASE_BOARD) || defined (USE_SIB_BASE_BOARD)
void bsp_usart1_send_data(uint8_t *data, uint8_t size)
{
	HAL_UART_Transmit(&husart1, (uint8_t *)&data[0], size, 0xFFFF);
}

uint16_t bsp_usart1_rx_data(uint8_t *data)
{
	uint16_t size=0;
	uint32_t timeout=0;
	
	HAL_UART_Receive_IT(&husart1, (uint8_t *)aRxBuffer,1); 
	...
	{ 
		...
		if(IsUsart1RxPending == SET)
		{
			IsUsart1RxPending = RESET;
			
			data[size++]=aRxBuffer[0];	
      HAL_UART_Receive_IT(&husart1, (uint8_t *)aRxBuffer,1);			
		}
	}
